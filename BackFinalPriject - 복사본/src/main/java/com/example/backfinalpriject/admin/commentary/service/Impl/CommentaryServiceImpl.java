...
public class CommentaryServiceImpl implements CommentaryService {
    ...
    public Long create(CommentaryRequest request, MultipartFile instructorImg, MultipartFile file, List<VideoUrlRequest> videoUrlRequest) throws Exception {
        Division division = divisionRepository.findByDivisionNameContaining(request.getDivision()).orElseThrow(()->new GlobalException(ErrorCode.DATABASE_ERROR, "division 오류"));
        Subject subject = subjectRepository.findByDivisionAndSubjectNameContaining(division, request.getSubject()).orElseThrow(()->new GlobalException(ErrorCode.DATABASE_ERROR, "과목 오류"));
    ...
    public CommentaryResponse getCommentaryDetail(Long id){
        ...
        CommentaryFile commentaryFile = commentaryFileRepository.findByIdNumber(commentary.getId()).orElseThrow(null);
